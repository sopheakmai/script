  ...
  setup() {
    ...
    const handleDragOver = (e: DragEvent, itemId?: string) => {
      ...
      if (targetItem.type === "parent") {
        // On the left side, we don't allow nesting of items
        if (target === "left") {
          // Do not show parent drop indicator for left side
          isDraggingOverParent.value = null;
        } else {
          // Prevent "single" type items from being added as children on right side
      ...
      // Use the target container already determined above
      ...
      return (
        <div class="flex-1 bg-gray-100 p-6 rounded-xl shadow-md transition-all duration-300 hover:shadow-lg" data-portal={target}>
          <h2 class="text-xl font-bold mb-4 text-gray-800 flex items-center">
            ...
            {target === "left" ? "Simple Flat List" : "Hierarchical List"}
          </h2>
          ...
          <div class="mt-3 transition-all duration-300 ease-in-out transform hover:scale-105">
            <div class={["p-3 rounded-lg text-xs border shadow-sm", target === "left"
            ...
            >
              ...
              {target === "left"
                ? (
                    <p>Left side shows a flat list of all items. All items are displayed at the same level without parent-child relationships.</p>
                  )
                : (
                    <p>Right side supports full hierarchy. Drop items in the center of a parent to add as a child.</p>
                  )}
            </div>
          </div>
        </div>
    ...
    return () => (
      <div class="max-w-7xl mx-auto p-6 bg-gradient-to-br from-blue-50 to-purple-50 min-h-screen">
        ...
        <div class="text-center mb-8">
          ...
          <p class="text-gray-600">Drag and reorder items between portals</p>
          <div class="mt-3 flex justify-center gap-4 text-sm">
            <div class="bg-blue-100 text-blue-800 px-3 py-1 rounded-full">
              ...
              {" "}
              Simple flat list (all items at same level)
            </div>
            <div class="bg-orange-100 text-orange-800 px-3 py-1 rounded-full">
              <span class="font-semibold">Right side:</span>
              {" "}
              Full hierarchy with parent/child items
            </div>
          </div>
        </div>
