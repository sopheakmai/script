...
import "./menu-drag-drop.css";

type Item = {
  id: string;
  name: string;
  type: "category" | "menu" | "heading";
  order: number;
  children: Item[];
};
...
export default defineComponent({
  ...
  setup() {
    const leftItems = ref<Item[]>([
      ...
      { id: "4-1", name: "Child 3.1", type: "menu", order: 0, children: [] },
      { id: "4-2", name: "Child 3.2", type: "menu", order: 1, children: [] },
      { id: "4-3", name: "Child 3.3", type: "menu", order: 2, children: [] },
      {
        id: "4",
        name: "Parent Item 13",
        type: "category",
    ...
    const restoreItemToSource = (item: Item, parentId: string | null, source: "left" | "right" | null) => {
      if (!source)
        return;

      const itemToRestore = deepCopyItem(item);

      if (source === "left") {
        if (parentId) {
          const parent = findItemById(leftItems.value, parentId);
          if (parent) {
            // Add back as a child
            parent.children.push(itemToRestore);
          } else {
            // Add back to the root level
            leftItems.value.push(itemToRestore);
          }
        } else {
          // Add back to the root level
          leftItems.value.push(itemToRestore);
        ...
        if (parentId) {
          const parent = findItemById(rightItems.value, parentId);
          if (parent) {
            // Add back as a child
            parent.children.push(itemToRestore);
          } else {
            // Add back to the root level
            rightItems.value.push(itemToRestore);
          }
        } else {
          // Add back to the root level
          rightItems.value.push(itemToRestore);
      ...
      console.warn(`Restored item ${itemToRestore.name} to ${source} side${parentId ? ` under parent ${parentId}` : ""}`);
    };
    ...
    const handleDragStart = (e: DragEvent, item: Item, source: "left" | "right", parentId: string | null = null) => {
      ...
      if (e.dataTransfer) {
        ...
        dragPreview.className = "drag-preview simple-preview";
      ...
      if (e.dataTransfer) {
        ...
        dragPreview.innerHTML = `
          <div class="drag-preview-content">
            <span class="drag-preview-indicator ${
              item.type === "category"
                ? "drag-preview-indicator-parent"
                : item.type === "menu"
                  ? "drag-preview-indicator-child"
                  : "drag-preview-indicator-single"
            }"></span>
            <span class="item-title">${item.name}</span>
            <span class="drag-preview-badge">${item.type}</span>
          </div>
        `;
        document.body.appendChild(dragPreview);
    ...
    const handleDragOver = (e: DragEvent, itemId?: string) => {
      ...
      if (topPart) {
      ...
      } else if (middlePart && targetItem.type === "category") {
        // Drop inside parent
        dropPosition.value = null;
        ...
        if (
          draggedItem.value.type === "category"
        ...
        // Check if trying to place a parent under another parent
        else if (draggedItem.value.type === "category") {
          targetElement.classList.add("forbidden-drop");
          targetElement.classList.add("parent-parent-forbidden");
          statusMessage.value = "Cannot add parent items under other parents";
        }
        else {
    ...
    const handleDragEnd = () => {
      ...
      // Remove parent-parent forbidden markers
      document.querySelectorAll(".parent-parent-forbidden").forEach((el) => {
        el.classList.remove("parent-parent-forbidden");
      });
    ...
    const removeItemFromSource = (itemId: string, parentId: string | null = null): boolean => {
      ...
      if (parentId) {
        ...
        if (itemToRemove && itemToRemove.type === "category" && itemToRemove.children.length > 0) {
    ...
    const addItemToParent = (target: "left" | "right", item: Item, parentId: string): boolean => {
      ...
      // Prevent adding "heading" type items as children
      if (itemToAdd.type === "heading") {
        statusMessage.value = "Cannot add single items as children";
        return false;
      }

      // Prevent adding "category" type items under other parents
      if (itemToAdd.type === "category") {
        statusMessage.value = "Cannot add parent items under other parents";
        console.warn("Attempted to add parent under parent:", itemToAdd.id, "under", parentId);
        return false;
      }
      ...
      // Double-check parent type (defensive programming)
      if (parent.type !== "category") {
        console.warn("Cannot add to non-parent item:", parent.type);
        return false;
      }
    ...
    const addItemAtPosition = (
      ...
      newArray.splice(posIndex, 0, {
        ...
        ...(itemToAdd.type !== "heading" ? { parentId: null } : {}),
      });
    ...
    const handleDrop = (e: DragEvent, target: "left" | "right", targetParentId: string | null = null) => {
      ...
      if (currentParentTarget) {
        ...
        // Prevent "heading" type items from being added as children
        ...
        // Prevent parent items from being added under other parent items
        ...
        if (!added) {
          ...
          // Restore the item to its original position if adding failed
          restoreItemToSource(itemToMove, draggedParentId.value, draggedFrom.value);
        } else {
          const parent = findItemById((target as string) === "left" ? leftItems.value : rightItems.value, currentParentTarget);
          statusMessage.value = `Added "${itemToMove.name}" as child of "${parent?.name || "category"}"`;
          lastAction.value = statusMessage.value;
        }
      ...
      if (currentDropPosition) {
        ...
        if (itemToMove.type === "heading" && targetParentId !== null) {
          statusMessage.value = `Cannot add single items as children. Use at top level only.`;
          lastAction.value = statusMessage.value;
        ...
        // Check if trying to place a parent under a parent item through dropping adjacent to its child
        if (itemToMove.type === "category" && targetParentId !== null) {
          // Check if the target parent exists
          const targetParent = findItemById(
        ...
        if (!added) {
          ...
          statusMessage.value = `Cannot place item at this position`;
        } else {
      ...
      if (targetParentId) {
        // Prevent "heading" type items from being added to parents
        if (itemToMove.type === "heading") {
          statusMessage.value = `Cannot add single items as children. Use at top level only.`;
          lastAction.value = statusMessage.value;
        ...
        return (
          <div key={item.id} class="item-container">
            ...
            <div
              ...
              onDrop={e => handleDrop(e as DragEvent, source, item.type === "category" ? item.id : parentId)}
              class={[
                "item",
                item.type === "category" ? "parent-item" : "",
                item.type === "menu" ? "child-item" : "",
                item.type === "heading" ? "single-item" : "",
                  ...
                  <div
                    class={[
                      ...
                      item.type === "category"
                        ...
                        : item.type === "menu"
                          ? "child-badge"
                          : "single-badge single-type-badge",
                    ]}
                  >
                    {item.type === "heading" ? "heading" : item.type}
                  </div>
                </div>
              </div>
      ...
      return (
        <div key={item.id} class="item-container">
          ...
          >
            ...
            {hasChildren && (
              <button onClick={() => toggleExpanded(item.id)} class="expand-button">
                <svg
                  class={["expand-button-icon", isExpanded ? "expanded" : ""]}
                  fill="currentColor"
                  viewBox="0 0 20 20"
                >
                  <path
            ...
            <div class="flex-1">
              <div class="item-content">
                ...
                <div class="item-meta">
                  ...
                  >
                    {item.type === "heading" ? "heading (top-level only)" : item.type}
                  </div>
                  <div class="item-order-badge">
                    <span>
                      Order:
                      {item.order}
                    </span>
            ...
            {" "}
            {isParentTarget && (
              <div class="parent-target-indicator">
                <div class="parent-target-message">
    ...
    const renderPortal = (items: Item[], target: "left" | "right") => {
      ...
      return (
        <div class="portal-container" data-portal={target}>
          <h2 class="portal-title">
            <div class={["portal-indicator", target === "left" ? "portal-indicator-left" : "portal-indicator-right"]} />
